...
async function getStoryParentComment(connection, storyIdx, userIdx, startItemIdx, pageItemCnt){
  ...
  SELECT SC.storyCommentIdx                    as commentIdx,
        ...
        CASE
            ...
            WHEN TIMESTAMPDIFF(day, DATE_FORMAT(SC.createdAt, '%Y-%m-%d'), DATE_FORMAT(NOW(), '%Y-%m-%d')) <= 3
                THEN CONCAT(TIMESTAMPDIFF(day, DATE_FORMAT(SC.createdAt, '%Y-%m-%d'), DATE_FORMAT(NOW(), '%Y-%m-%d')),
                            '일 전')
  ...
  FROM StoryComment SC
          ...
          LEFT JOIN (SELECT parentIdx, COUNT(storyCommentIdx) AS childCommentCnt
                      FROM StoryComment
                      WHERE isDeleted = 'N' && parentIdx IS NOT NULL
                      GROUP BY parentIdx) SCC ON SCC.parentIdx = SC.storyCommentIdx
  WHERE SC.parentIdx IS NULL && SC.storyIdx = ${storyIdx} &&
        (SC.isDeleted = 'N' || (SC.isDeleted = 'Y' && childCommentCnt IS NOT NULL))
...
//스토리 댓글 신고
async function reportStoryComment(connection, storyCommentIdx, userIdx, reportedReasonIdx, etcReason, commentTypeIdx){
  const query = `
  INSERT INTO ReqReportedComment(commentTypeIdx, commentIdx, userIdx, reportedReasonIdx, etcReason)
  VALUES (${commentTypeIdx}, ${storyCommentIdx}, ${userIdx}, ${reportedReasonIdx}, ?);
  `;
...
async function isAlreadyReportedStoryCommentIdx(connection, userIdx, storyCommentIdx, commentTypeIdx){
  const query = `
  SELECT EXISTS(SELECT *
    FROM ReqReportedComment
    WHERE userIdx = ${userIdx} && commentIdx = ${storyCommentIdx} && commentTypeIdx = ${commentTypeIdx}) as isExist;
  `;
  const [rows] = await connection.query(query);
  return rows[0]['isExist'];
}
